...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "myForm")) {
  $updateSQL = sprintf("UPDATE users SET username=%s, password=%s, fname=%s, lname=%s, email=%s, dob=%s, city=%s, region=%s WHERE id=%s", 
...
$colname_manageuser = "-1";
if (isset($_SERVER['username'])) {
  $colname_manageuser = $_SERVER['username'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_manageuser = sprintf("SELECT * FROM users WHERE username = %s", GetSQLValueString($colname_manageuser, "text"));
$manageuser = mysql_query($query_manageuser, $koneksi) or die(mysql_error());
$row_manageuser = mysql_fetch_assoc($manageuser);
$colname_manageuser = "-1";
if (isset($_COOKIE['id'])) {
  $colname_manageuser = $_COOKIE['id'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_manageuser = sprintf("SELECT * FROM users WHERE id = %s", GetSQLValueString($colname_manageuser, "int"));
$manageuser = mysql_query($query_manageuser, $koneksi) or die(mysql_error());
$row_manageuser = mysql_fetch_assoc($manageuser);
mysql_select_db($database_koneksi, $koneksi);
$query_manageuser = "SELECT * FROM users WHERE id = id";
$manageuser = mysql_query($query_manageuser, $koneksi) or die(mysql_error());
$row_manageuser = mysql_fetch_assoc($manageuser);
   ...
   <li class='active has-sub'><a href='#'><span>ALL DATA</span></a>
    ...
    <div id="isi">
  	<article>
        ...
        <p>&nbsp;</p>
    	<h3><center>
    	  <p>Fill you identity and join us!    	</p>
    	</center>
    	</h3>
...
<span class="textfieldRequiredMsg">A value is required.</span></span></td>
		  ...
		  <tr>
		    <th scope="row"></th>
		    <td><input name="id" type="hidden" id="id" value="<?php echo $row_manageuser['id']; ?>"></td>
